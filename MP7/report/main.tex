...
\begin{myindentpar}{5mm}
	...
	\begin{itemize}
	    ...
	    \item The program should function correctly for any number of threads. Practically speaking, this means that if the thread creation function call fails (see Synchronization under Advanced Concepts), or the RequestChannel constructor fails (it will throw an exception if it does), your program must recover and finish correctly (if such errors prevent any worker threads from being started, report the problem and exit). Check the return value of of the thread creation function call (also check errno for valuable debug information, man 3 errno), and catch any exceptions thrown by the RequestChannel constructor, "cleaning up" as required. Further error-handling than that should not be needed. Note that failures to create threads or construct RequestChannels do not always (at least, not in and of themselves) indicate a programming error, but may be due to OS limitations such as maximum threads per process or available file descriptors (both of which vary greatly among operating systems).
	    ...
	    \item Your code may not make use of any global variables, as they (in most circumstances) exemplify poor programming practice. Appropriately-named storage structs have been provided to be used in their place, although you will need to fill them in before they can be used.
