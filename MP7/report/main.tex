...
\begin{myindentpar}{5mm}
	...
	\begin{itemize}
	    ...
	    \item The program should function correctly for any number of threads. Practically speaking, this means that if the pthread\_create function call fails (see Synchronization under Advanced Concepts), or the RequestChannel constructor fails (it will throw an exception if it does), your program must recover and finish correctly (if such errors prevent any worker threads from being started, report the problem and exit). Check the return value of of the pthread\_create function call (also check errno for valuable debug information, man 3 errno) and catch any exceptions thrown by the RequestChannel constructor, in both cases "cleaning up" as required. Further error-handling should not be needed. Note that failures to create threads or construct RequestChannels do not always (at least, not in and of themselves) indicate a programming error but may be due to OS limitations such as maximum threads per process or available file descriptors, both of which vary greatly among operating systems.
	    ...
	    \item Your code may not make use of \emph{any} global variables, as they (in most circumstances) exemplify poor programming practice. Appropriately-named storage structs have been provided to be used in their place, although you will need to fill in their code before using them.
...
\begin{itemize}
        ...
        \item Request Threads (20 points)
			...
			\item Coded one, \emph{and only one}, request\_thread\_function, and not 3 separate copies of it (one for each user). If 3 such copies are used, deduct 10 points. The person's name must be passed as an argument, or as one of the member variables of PARAMS\_REQUEST.
	...
	\item Worker Thread Functionality (30 points)
		...
		\setlength\itemsep{-0.1em}
			...
			\item Histograms correct. (10 points)
			\item Proper thread-safety used for request\_buffer and histogram vectors. (10 points
				\begin{itemize}
				\setlength\itemsep{-0.1em}
					\item SafeBuffer used for request\_buffer.
					\item Locks used before incrementing histogram vectors.
				\end{itemize}
		\end{enumerate}
	\item PARAMS (10 points)
		\begin{enumerate}
        		\setlength\itemsep{-0.1em}
