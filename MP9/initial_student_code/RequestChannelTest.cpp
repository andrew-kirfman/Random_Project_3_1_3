...
int main(int argc, char * argv[]) {
			...
			case 'b':
				...
				if(b != PTHREAD_BUFFER &&
				   ...
				   b != SHARED_MEMORY_BUFFER) {
					...
					/*
						Remove or comment out this else block if you decide to implement/use
						MessageQueueBoundedBuffer or SharedMemoryBoundedBuffer.
					 */
					b = PTHREAD_BUFFER;
				}
				break;
	...
	if((!SEPARATE_WINDOWS && pid > 0) ||
	   SEPARATE_WINDOWS){
		...
		/*
		 If you need to impose upper bounds on certain parameters
		 due to system limitations, this is a good place to do so
		 */
